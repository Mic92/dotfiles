...
if [[ -n ${commands[fzf]} ]]; then
  ...
  # Tokyo Night LS_COLORS (pre-generated by `vivid generate tokyonight-night`).
  # Static file avoids a runtime dep on vivid; regenerate when palette changes.
...
sieve-edit() {
  ...
  nix run nixpkgs#sieve-connect -- --passwordfd $passwordfd -s imap.thalheim.io -u joerg@thalheim.io --remotesieve dovecot --edit
  exec {passwordfd}>&-
}
...
if [[ -f ~/.fast-syntax-highlighting/fast-syntax-highlighting.plugin.zsh ]]; then
  ...
  # Apply Tokyo Night once; fsh persists the choice in its cache so only redo
  # when the theme name differs (cheap [[ ]] vs ~40ms fast-theme call).
  # Theme lives outside the fsh submodule so it can be tracked here.
  if [[ ${FAST_THEME_NAME:-} != tokyonight && -r ~/.config/fsh/tokyonight.ini ]]; then
    fast-theme ~/.config/fsh/tokyonight.ini >/dev/null 2>&1
  fi
fi
