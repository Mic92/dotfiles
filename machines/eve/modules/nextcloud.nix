{ config, pkgs, ... }:
{
  ...
  systemd.services.nextcloud-ldap-config = {
    ...
    script = ''
      ...
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapPort "3890"
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapAgentName "uid=nextcloud,ou=people,dc=eve"
      ...
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapBase "dc=eve"
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapBaseUsers "ou=people,dc=eve"
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapUserFilter "(memberOf=cn=nextcloud,ou=groups,dc=eve)"
      ...
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapLoginFilter "(&(memberOf=cn=nextcloud,ou=groups,dc=eve)(mail=%uid))"
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapExpertUsernameAttr "mail"
      ${config.services.nextcloud.occ}/bin/nextcloud-occ ldap:set-config "" ldapExpertUUIDUserAttr "entryuuid"
