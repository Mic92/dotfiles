...
local build = {
  ...
  steps: [{
    ...
    commands: [
      ...
      'LOGNAME=drone nix run .#irc-announce -- 10.243.77.3 6667 drone "#xxx" "build $DRONE_SYSTEM_PROTO://$DRONE_SYSTEM_HOST/$DRONE_REPO/$DRONE_BUILD_NUMBER: $DRONE_BUILD_STATUS" || true'
    ],
    when: {
      event: { exclude: ['pull_request'] },
      status: ['failure', 'success'],
