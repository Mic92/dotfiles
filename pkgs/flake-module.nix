...
in
{
  ...
  // lib.optionalAttrs pkgs.stdenv.isLinux {
    ...
    # nginx with the QUIC preread stream module statically linked.
    nginx-quic-preread = pkgs.nginx.override {
      modules = [ (pkgs.callPackage ./nginx-quic-preread/package.nix { }) ];
    };
    # The Rust crate; its build runs the unit + real-nginx integration tests.
    ngx-quic-preread = pkgs.callPackage ./nginx-quic-preread/crate.nix { };
