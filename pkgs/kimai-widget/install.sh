...
WIDGET_ID="com.thalheim.kimai-widget"

echo "Installing Kimai Widget..."
...
# Substitute the API token placeholder in main.qml.
if [ $# -lt 1 ]; then
  echo "Usage: $0 <api-token>" >&2
  echo "Example: $0 \"\$(rbw get 'Kimai API')\"" >&2
  exit 1
fi
token="$1"
sed -i "s/@KIMAI_API_TOKEN@/$token/" \
  "$PLASMOID_DIR/$WIDGET_ID/contents/ui/main.qml"
...
echo "3. Search for 'Kimai Time Tracker'"
