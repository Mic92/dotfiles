...
class Pinentry:
    ...
    def _show_zenity_password_dialog(
        ...
        try:
            ...
            result = subprocess.run(zenity_cmd, capture_output=True, text=True, check=False)
    ...
    def _handle_master_password(self, error: str) -> str | None:
        if error:
            ...
            cached_password = self.backend.get_password(self.service_name, self.rbw_profile)
        ...
        secret_value = self._show_zenity_password_dialog(title=title, prompt=prompt, desc=desc, error=error)
        if secret_value and not error:
            self.backend.store_password(self.service_name, self.rbw_profile, secret_value)
        return secret_value

    def _handle_other_password(self, title: str, prompt: str, desc: str, error: str) -> str | None:
        return self._show_zenity_password_dialog(title=title, prompt=prompt, desc=desc, error=error)
    ...
    def _handle_getpin(self, state: dict[str, str]) -> str:
        if state["prompt"] == "Master Password":
            ...
            secret_value = self._handle_other_password(state["title"], state["prompt"], state["desc"], state["error"])
        if secret_value:
            print(f"D {secret_value}", flush=True)
        return "OK"

    def _process_command(self, command: str, args: str, state: dict[str, str]) -> str | None:
        command_handlers = {
